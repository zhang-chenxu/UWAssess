...
from models_sam2 import register
from .sam2.modeling.sam2_maskdecoder_twowaytransformer_myself import MaskDecoder, TwoWayTransformer

logger = logging.getLogger(__name__)
from .iou_loss import IOU
from typing import Any, Optional, Tuple
from .sam2.modeling.backbones.image_encoder_stochasticdepth import ImageEncoder
from .sam2.modeling.backbones.hieradet_LoraAdapterexternal import Hiera
from .sam2.modeling.backbones.image_encoder_stochasticdepth import FpnNeck
...
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None,
        ...
        self.image_encoder = ImageEncoder(
            trunk=Hiera(embed_dim=encoder_mode['trunk']['embed_dim'],
                        embed_dims=encoder_mode['trunk']['embed_dims'],
                        num_heads=encoder_mode['trunk']['num_heads'],
                        stages=encoder_mode['trunk']['stages'],
                        global_att_blocks=encoder_mode['trunk']['global_att_blocks'],
                        window_pos_embed_bkg_spatial_size=encoder_mode['trunk']['window_pos_embed_bkg_spatial_size'],
                        window_spec=encoder_mode['trunk']['window_spec']
                        ),
            neck=FpnNeck(d_model=encoder_mode['neck']['d_model'],
                         backbone_channel_list=encoder_mode['neck']['backbone_channel_list'],
                         fpn_top_down_levels=encoder_mode['neck']['fpn_top_down_levels'],
                         fpn_interp_model=encoder_mode['neck']['fpn_interp_model']
                         ),
        )
        self._bb_feat_sizes = [
            (256, 256),
    ...
    def forward(self, input, comp_drop=False, training=False):
        bs = input.shape[0]
        ...
        if self.directly_add_no_mem_embed:
            vision_feats[-1] = vision_feats[-1] + torch.nn.Parameter(torch.zeros(1, 1, 256).to(input.device))  # (4096,batch,256)
        feats = [
            feat.permute(1, 2, 0).view(vision_feats[0].shape[1], -1, *feat_size)
            for feat, feat_size in zip(vision_feats[::-1], self._bb_feat_sizes[::-1])
        ][::-1]   # [(batch,32,256,256) (batch,64,128,128) (batch,256,64,64)]

        if comp_drop:
            for i in range(len(feats)):
                feature = feats[i]
                batch_size, channel_dim = feature.shape[0], feature.shape[1]

                dropout_mask1 = self.binomial.sample((batch_size // 2, channel_dim)).to(feature.device) * 2.0
                dropout_mask2 = 2.0 - dropout_mask1
                dropout_prob = 0.5
                num_kept = int(batch_size // 2 * (1 - dropout_prob))
                kept_indexes = torch.randperm(batch_size // 2)[:num_kept]
                dropout_mask1[kept_indexes, :] = 1.0
                dropout_mask2[kept_indexes, :] = 1.0

                dropout_mask = torch.cat((dropout_mask1, dropout_mask2))

                feats[i] = feature * dropout_mask.unsqueeze(2).unsqueeze(3)
