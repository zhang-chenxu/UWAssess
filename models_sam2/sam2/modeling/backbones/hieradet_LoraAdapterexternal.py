...
class MultiScaleAttention(nn.Module):
    def __init__(
        ...
        self.loralayer = LoRALayer(dim, dim_out, 4, 8, num_heads)
    ...
    def forward(self, x: torch.Tensor, gate_input: torch.Tensor) -> torch.Tensor:
        ...
        lora_q, lora_v = self.loralayer(x, gate_input)
        q = q + lora_q
        v = v + lora_v
        ...
        # x = F.scaled_dot_product_attention(        # for pytorch >= 2.0.0
        x = scaled_dot_product_attention(            # for pytorch < 2.0.0
            q.transpose(1, 2),
            k.transpose(1, 2),
            v.transpose(1, 2),
        )
...
class Hiera(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        ...
        for i, blk in enumerate(self.blocks):
            ...
            elif self.depth[0] < i < self.depth[0] + self.depth[1] + 1:
                self.block2.append(blk)  # 第二个块包含接下来的6个元素
            elif self.depth[0] + self.depth[1] < i < self.depth[0] + self.depth[1] + self.depth[2] + 1:
                self.block3.append(blk)  # 第三个块包含接下来的36个元素
            elif self.depth[0] + self.depth[1] + self.depth[2] < i:
        ...
        for i, blk in enumerate(self.block1):
            ...
            stage_count += 1
        # x = self.norm1(x)
            if (stage_count == self.stage_ends[-1]) or (
                stage_count in self.stage_ends and self.return_interm_layers
            ):
...
class PromptGenerator(nn.Module):
        ...
        if self.adaptor == 'adaptor':
            if '1' in self.tuning_stage:
                for i in range(self.depths[0]+1):
                    ...
                    external_adapter_gate = Gate(self.embed_dims[0] // self.scale_factor)
                    setattr(self, 'external_adapter_gate1_{}'.format(str(i)), external_adapter_gate)
            ...
            if '2' in self.tuning_stage:
                for i in range(self.depths[1]+1):
                    ...
                    external_adapter_gate = Gate(self.embed_dims[1] // self.scale_factor)
                    setattr(self, 'external_adapter_gate2_{}'.format(str(i)), external_adapter_gate)
            ...
            if '3' in self.tuning_stage:
                for i in range(self.depths[2]+1):
                    ...
                    external_adapter_gate = Gate(self.embed_dims[2] // self.scale_factor)
                    setattr(self, 'external_adapter_gate3_{}'.format(str(i)), external_adapter_gate)
            ...
            if '4' in self.tuning_stage:
                for i in range(self.depths[3]+1):
                    ...
                    external_adapter_gate = Gate(self.embed_dims[3] // self.scale_factor)
                    setattr(self, 'external_adapter_gate4_{}'.format(str(i)), external_adapter_gate)
    ...
    def get_prompt(self, x, prompt, block_num, depth_num):
        ...
        if self.adaptor == 'adaptor':
            ...
            external_adapter_gate = getattr(self, 'external_adapter_gate' + str(block_num) + '_' + str(depth_num))
            shared_mlp = getattr(self, 'shared_mlp' + str(block_num))

            feat = lightweight_mlp(feat)
            scale = external_adapter_gate(feat)
...
class LoRALayer(nn.Module):
    def __init__(self, in_dim, out_dim, rank, alpha, num_heads):
        '''
        ...
        self.A_q = nn.Parameter(torch.randn(in_dim, rank) * std_dev, requires_grad=True)
        self.B_q = nn.Parameter(torch.zeros(rank, out_dim), requires_grad=True)
        self.A_v = nn.Parameter(torch.randn(in_dim, rank) * std_dev, requires_grad=True)
        self.B_v = nn.Parameter(torch.zeros(rank, out_dim), requires_grad=True)
        self.num_heads = num_heads
        self.gate = Gate(in_dim)
        self.scale = torch.tensor(alpha / rank).float()

    def forward(self, x, gate_input):
        B, H, W, _ = x.shape
        batchsize, _, _, _ = gate_input.shape
        scale = self.gate(gate_input) * self.scale
        blocks = int(B / batchsize)

        x_q = x @ self.A_q @ self.B_q
        x_q = x_q.reshape(B, H * W, self.num_heads, -1)

        x_v = x @ self.A_v @ self.B_v
        x_v = x_v.reshape(B, H * W, self.num_heads, -1)

        for i in range(batchsize):
            current_scale = scale[i, :, :, :]
            x_q[i * blocks: i * blocks + blocks, :, :, :] *= current_scale
            x_v[i * blocks: i * blocks + blocks, :, :, :] *= current_scale
...
class Gate(nn.Module):
    ...
    def forward(self, x):
        ...
        scale = self.gate(x).mean(dim=1, keepdim=True).mean(dim=2, keepdim=True)
